...
class GMMClassifier:
    ...
    def add_profile(self, label='', files=''):
        ...
        if label == 'ubm':
            gmm = GMM(n_components=self.n_ubm_components,covariance_type=self.ubm_covariance_type,max_iter=self.max_iter)
        else:
            gmm = GMM(n_components=self.n_components,covariance_type=self.covariance_type,max_iter=self.max_iter)
    ...
    def calculate_features(self, signal, sample_rate):
        ...
        numcep=self.n_ccs,nfft=int(self.win_len*self.sample_rate),preemph=0,winlen=self.win_len, \
...
def main():
    ...
    with open(log_file,'w') as out:
        ...
        if test_standard:
            ...
            out.write(str(current_accuracy)+str(current_params)+str(train_corpus_dict)+str(test_corpus_dict)+'\n')
            #print(current_accuracy, current_params)

            # labels = classifier.get_classes()
            # d = classifier.get_dataset()
            # current_ccs = classifier.get_n_ccs()
            # fig, ax = plt.subplots()
            # for class_no in range(len(d)):
            #     class_data = d[class_no]
            #     color = 'C{0}'.format(class_no)
            #     for i in range(current_ccs*3):
            #         x_coord = 2*i + class_no
            #         x_axis = np.full((class_data.shape[0]),x_coord)
            #         plt.scatter(class_data[:,i], x_axis, c=color,label=labels[class_no])
            # ax.legend()
            # plt.show()
    ...
    if animate:
        ...
        target_podcast = 'all_podcasts/196.wav' # Not going to generalize this, make it a specific podcast
